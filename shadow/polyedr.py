...
class Polyedr:
    ...
    def __init__(self, file):
        ...
        self.edges = []             # рёбра для отрисовки (могут быть дубли)
        self.orig_edges_set = set() # множество уникальных пар индексов исходных рёбер
        self.unique_orig_edges = [] # список уникальных исходных рёбер (для характеристики)
        self.facets = []
        ...
        with open(file) as f:
            for i, line in enumerate(f):
                if i == 0:
                    ...
                    # Списки вершин (преобразованных и исходных)
                    verts = [self.vertexes[i] for i in indices]
                    orig_verts = [self.orig_vertexes[i] for i in indices]
                    # Добавляем рёбра для отрисовки (каждое ребро добавляется дважды)
                    for n in range(size):
                        self.edges.append(Edge(verts[n-1], verts[n]))
                    # Запоминаем уникальные рёбра в исходных координатах
                    for n in range(size):
                        i = indices[n-1]
                        j = indices[n]
                        if i < j:
                            self.orig_edges_set.add((i, j))
                        else:
                            self.orig_edges_set.add((j, i))
                    # Грань для отрисовки
                    self.facets.append(Facet(verts))

        # Создаём список уникальных исходных рёбер
        for i, j in self.orig_edges_set:
            self.unique_orig_edges.append(Edge(self.orig_vertexes[i], self.orig_vertexes[j]))
    ...
    def good_edges_projection_length(self):
        """Сумма длин проекций уникальных рёбер, середины которых в исходных координатах — «хорошие» точки."""
        total = 0.0
        for e in self.unique_orig_edges:
            mid = (e.beg + e.fin) * 0.5
            if self.is_good_point(mid):
                dx = e.fin.x - e.beg.x
                dy = e.fin.y - e.beg.y
