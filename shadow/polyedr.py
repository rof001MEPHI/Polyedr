...
class Polyedr:
    ...
    @staticmethod
    def is_good_point(p):
        """Проверяет, является ли точка p «хорошей»."""
    ...
    def good_edges_projection_length(self):
        """Возвращает сумму длин проекций рёбер,
        середины которых — «хорошие» точки."""
        total = 0.0
        seen = set()          # множество уже учтённых рёбер
        for e in self.edges:
            # Ключ ребра – неупорядоченная пара вершин
            key = frozenset([e.beg, e.fin])
            if key in seen:
                continue
            seen.add(key)

            mid = (e.beg + e.fin) * 0.5   # середина ребра
